...
/**
 * is_chain - checks if we have a chain delimeter 
 ...
 * Return: Void
 */
void chain_checker(info_t *data, char *buf, size_t *p, size_t i, size_t len)
{
	size_t j = *p;
	...
	for (i = 0; data->argv[i]; i++)
	{
		...
		if (!_strcmp(data->argv[i], "$?"))
		{
			replace_string(&(data->argv[i]),
					_strdup(num_convert(data->status, 10, 0)));
			continue;
		}
		if (!_strcmp(data->argv[i], "$$"))
		{
			replace_string(&(data->argv[i]),
					_strdup(num_convert(getpid(), 10, 0)));
			continue;
		}
